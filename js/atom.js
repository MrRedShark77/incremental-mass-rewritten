const ATOM = {
    ...
    quarkGain() {
        ...
        let o = x
        let os = tmp.c16active && OURO.evo < 2 ? E('ee6') : E(OURO.evo >= 2 ? 'ee70' : 'ee90').pow(treeEff('ct13')?tmp.chal.eff[15]:1), op = E(.5)
    ...
    particles: {
        ...
        effect(i) {
            ...
            let x = p.pow(2)
            if (hasElement(12)) x = p.pow(p.add(1).log10().add(1).root(4).pow(OURO.evo >= 3 ? 1 : tmp.chal.eff[9]).softcap(40000,0.1,0))
