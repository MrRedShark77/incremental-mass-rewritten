const BEYOND_PRES = {
    req() {
        let x = player.pres.beyond.pow(1.25).mul(10).add(12).ceil()
        return x
    },
    bulk() {
        let x = player.prestiges[4].gte(180)?player.prestiges[4].sub(180).div(10).max(0).root(1.25).add(1).floor():E(0)
        return x
    },
    getTier() {
