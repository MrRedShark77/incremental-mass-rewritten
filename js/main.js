...
const FORMS = {
  ...
  massGain() {
    ...
    x = x.softcap(tmp.massSoftGain6, tmp.massSoftPower6, 0).softcap(tmp.massSoftGain7, tmp.massSoftPower7, 0).softcap(tmp.massSoftGain8, tmp.massSoftPower8, 0);
  ...
  tickspeed: {
    ...
    can() {
      return player.rp.points.gte(tmp.tickspeedCost) && !CHALS.inChal(2) && !CHALS.inChal(6) && !CHALS.inChal(10);
    },
    buy() {
      if (this.can()) {
  ...
  bh: {
    ...
    condenser: {
      ...
      can() {
        return player.bh.dm.gte(tmp.bh.condenser_cost) && !CHALS.inChal(6) && !CHALS.inChal(10);
      },
      buy() {
        if (this.can()) {
