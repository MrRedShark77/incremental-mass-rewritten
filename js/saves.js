...
function calc(dt, dt_offline) {
    ...
    if (tmp.pass) {
        ...
        if (hasElement(128)) for (let x = 0; x < MASS_DILATION.break.upgs.ids.length; x++) if (MASS_DILATION.break.upgs.ids[x].unl?MASS_DILATION.break.upgs.ids[x].unl():true) MASS_DILATION.break.upgs.buy(x)
        ...
        if (CHALS.inChal(0)) {
            ...
            if (hasTree("qu_qol10")) for (let x = 9; x <= 11; x++) player.chal.comps[x] = player.chal.comps[x].max(tmp.chal.bulk[x].min(tmp.chal.max[x]))
        }
    }
