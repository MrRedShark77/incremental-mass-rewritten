const SUPERNOVA = {
    ...
    req(x=player.supernova.times) {
        ...
        maxlimitGal = E('ee35000').pow(player.galaxy.times.scaleEvery('galaxy',false).pow(20000)).mul('ee28000')
        bulk = E(0)
        bulkGal = E(0)
        if (player.stars.points.div('ee28000').gte(1)) bulkGal = player.stars.points.div('ee28000').max(1).log('ee35000').max(0).root(20000).scaleEvery('galaxy',true).add(1).floor()
